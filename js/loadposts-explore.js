            ...
            postHtml+= '<div class="post-content">' +
                    ...
                    '<div class="complete-Post ">' +
                        '<div class="complete-post-image invisible">';
                            if(post.postUrl!=null){if(post.tipo=="imagen"){postHtml+='<div class="media"><img src="' + post.postUrl + '" /></div>';
                        }else if(post.tipo=="video"){postHtml+='<div class="media"><video src="' + post.postUrl + '" controls ></video></div>';}else if(post.tipo=="audio"){postHtml+='<div class="cortina-audio"><audio src="' + post.url_recurso + '" controls ></audio></div>';}else if(post.tipo=="archivo"){postHtml+='<div class="post-image">' + "<a class='post-archivo' href='" + post.postUrl + "'>"+ post.titulo +"<i class='fa-solid fa-file-arrow-down' style='color: #dbdbdb;'></i></a>" + '</div>';}};
                          postHtml+= '<div>'+
                          '<h2 style="text-align:center;">' + post.titulo + ' || ' + '<a href="perfil?id='+ post.usuarioId + '">' + post.nombre + '</h2></a>' +
